...
public class PagedCommitOutputHandler extends LineReaderOutputHandler implements CommandOutputHandler<Page<Commit>>
{
    ...
    public PagedCommitOutputHandler(Repository repository, CommitsCommandParameters parameters, PageRequest pageRequest) {
        ...
        this.commitReaderSettingsBuilder.repository(repository);
        this.commitReaderSettingsBuilder.withMessages(parameters.isWithMessages());
        this.commitReader = new CommitReader(this.commitReaderSettingsBuilder.build())
          {
            protected void ping()
            {
    ...
    public Page<Commit> getOutput() { return PageUtils.createPage(this.commits, this.pageRequest); }
  
    
    public CommitReader getCommitReader() { return this.commitReader; }

  
    protected Commit readCommit(LineReader reader) throws IOException { return this.commitReader.readCommit(reader); }
